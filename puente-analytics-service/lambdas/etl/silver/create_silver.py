...
def fill_tables(conn, get_dimensions=True, get_puente_tables=True, debug=False):
    ...
    if get_dimensions:
        ...
        print("users")
        get_users_dim(conn, users_df, debug)
        print("users dim")
        get_household_dim(conn, survey_df, debug)
        print("household dim")
        get_patient_dim(conn, survey_df, debug)
        print("patient dim")
        get_question_dim(conn, form_specs, debug)
        print("question dim")
    ...
    if get_puente_tables:
        for table_name, table_desc in NOSQL_TABLES.items():
            ...
            add_nosql_to_fact(conn, table_name, survey_df, debug)
            print("add to fact")
    ...
    get_custom_forms(conn, form_results, debug)
    print("custom forms")
